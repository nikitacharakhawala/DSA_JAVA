...
public class AllGCDPairs {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(getAllGCDPairs(A)));
    }

    public static int[] getAllGCDPairs(int[] A){
        int part=(int)Math.sqrt(A.length), max=Integer.MIN_VALUE, count=0, index=0;
        int[] allGCDPair=new int[part];
        for(int i=0;i<A.length;i++){
            max=Math.max(max,A[i]);
            count++;
            if(count==part){
                count=0;
                allGCDPair[index]=max;
                index++;
