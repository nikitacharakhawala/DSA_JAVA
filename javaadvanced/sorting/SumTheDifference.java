...
public class SumTheDifference {
    public static void main(String[] args) {
        ...
        System.out.println(getSumTheDifference(A));
    }

    public static int getSumTheDifference(int[] A){
        long max=0,min=0;
        int mod=1000000007;
        Arrays.sort(A);
        for(int i=0;i<A.length;i++){
            max=((max+(long)A[A.length-i-1]*pow(2,A.length-i-1,mod))%mod);
            min=((min+(long)A[i]*pow(2, A.length-i-1,mod))%mod);
        }
       // System.out.println(max+"::::"+min+"::::"+(max-min)%mod);
        return (int)(max-min+mod)%mod;
    }

    public static int pow(int a, int pow, int mod){
        if(pow==0)
            return 1;
        long halfPow=pow(a, pow/2, mod);
        if(pow%2==0)
            return (int)(((halfPow%mod)*(halfPow%mod))%mod);
        else
            return (int)(((halfPow%mod)*(halfPow%mod)*(a%mod))%mod);
    }
}
